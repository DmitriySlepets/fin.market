...
/**
 * Отберем свойства по инфоблоку "Управление параметрами продуктов" - НАЧАЛО
 */
$arrThisProp = array();
foreach ($arResult["PROPERTIES"] as $key=>$item)
{
    $arrThisProp[] = $item['NAME'];
}//foreach ($arResult["PROPERTIES"] as $key=>$item)

if(count($arrThisProp)>0)
{


    $arrVisibleProperty = array();

    $arFilter = ['IBLOCK_ID' => 37, 'ACTIVE' => 'Y', 'NAME'=>$arrThisProp,'PROPERTY_INCLUDE_GROUP'=>$arResult['IBLOCK_SECTION_ID']];
    $arSelect = [ 'ID', 'NAME', 'CODE'];
    $res = CIBlockElement::GetList(array(),$arFilter,false,false,$arSelect);
    while($itemRes = $res->Fetch()) {
        $arrVisibleProperty[] = $itemRes['NAME'];
    }
    ...
    if(count($arrVisibleProperty)>0)
    {
        ...
        foreach ($arResult["PROPERTIES"] as $key=>$item)
        {
            if(in_array($item['NAME'],$arrVisibleProperty))
            {
                $resultProperties[$key] = $item;
...
/**
 * Отберем свойства по инфоблоку "Управление параметрами продуктов" - КОНЕЦ
 */
foreach($arResult["PROPERTIES"] as &$arProps) {
	//see($arProps);break;
	$arElOrder = array("SORT" => "ASC");
	$arElFilter = array("IBLOCK_ID" => 37, "PROPERTY_CODE_PROP" => $arProps["CODE"], "ACTIVE" => "Y");
   	$arElSelect = array("IBLOCK_ID","ID", "PROPERTY_*");
     	$res = CIBlockElement::GetList($arElOrder, $arElFilter, false, false, $arElSelect);
	//see($res);
	$result = $res->GetNextElement();
	//see($result);
	if(!empty($result)){
		$resAllProps = $result->GetProperties();
		if($resAllProps["TAGVIEW"]["VALUE"] == "Да"){
			$arProps["SHOW_IN_TAGS"] = 'N';
		} else {
			$arProps["SHOW_IN_TAGS"] = 'Y';
		}
	}
}
//Сопоставление картинки и типа платежной системы
	$arResult["listOfVariants"] = Array();
	$property_enums = CIBlockPropertyEnum::GetList(Array("DEF"=>"DESC", "SORT"=>"ASC"), Array("IBLOCK_ID"=>35, "CODE"=>"HARAKTERISTIKI_PLATEZHNAYA_SISTEMA"));
	while($enum_fields = $property_enums->GetNext()) {
		$arResult["listOfVariants"][] = $enum_fields["VALUE"];
	}
	//see($arResult["listOfVariants"]);
	$arImgOrder = array("SORT" => "ASC");
     	$arImgFilter = array("IBLOCK_ID" => 37, "PROPERTY_CODE_PROP" => "HARAKTERISTIKI_PLATEZHNAYA_SISTEMA", "ACTIVE" => "Y");
     	$arImgSelect = array("IBLOCK_ID","ID", "PROPERTY_*");
     	$resImg = CIBlockElement::GetList($arImgOrder, $arImgFilter, false, false, $arImgSelect);
	$resultImg = $resImg->GetNextElement();
	$resImgProp = $resultImg->GetProperties();
	$arResult["resultImgProp"] = $resImgProp["PIC"];
	foreach($arResult["resultImgProp"]["VALUE"] as &$valueImgProp) {
		$valueImgProp = CFile::GetPath($valueImgProp);
	}
	//see($arResult["resultImgProp"]);
	//see($arResult["ORIGINAL_PARAMETERS"]["SECTION_CODE"]);
?>