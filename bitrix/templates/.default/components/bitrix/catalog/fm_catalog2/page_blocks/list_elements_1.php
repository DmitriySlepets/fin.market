...
<?if($itemsCnt):?>
			...
			<?}?>
                ...
                if (isset($_GET["sort"]) && isset($_GET["method"]) && (
                        $_GET["sort"] == "property_MAIN_PROZ_STAVKA" || $_GET["sort"] == "property_MAIN_KREDIT_LIMIT" || $_GET["sort"] == "property_MAIN_LGOTNIY_PERIOD" || $_GET["sort"] == "property_MAIN_GOD_OBSL")){
                    $sort = $_GET["sort"];
                    ...
                    if($pos_credit_cart>0){
                        $sort = "property_MAIN_KREDIT_LIMIT";
                        $sort_order = "desc";
                    }
                    if($pos_credit_nal>0){
                        $sort = "property_MAIN_PROZ_STAVKA";
                        $sort_order = "desc";
                    }
                    if($pos_debit_cart>0){
                        $sort = "property_MAIN_GOD_OBSL";
                        $sort_order = "desc";
                    }
                    if($pos_rko>0){
                        $sort = "property_MAIN_GOD_OBSL";
                ...
                $listElementsTemplate = "fm_" . $listElementsTemplate . "_catalog2";
                ?>
					...
					<?if($arParams["SHOW_SECTION_DESC"] != 'N' && strpos($_SERVER['REQUEST_URI'], 'PAGEN') === false):?>
						...
						<?if($posSectionDescr === "BOTH"):?>
							<?if($arSection[$section_pos_bottom]):?>
								<div class="group_description_block bottom elastic_block off">
